...
import { adminAboutFeatureHighlightService } from '../services/aboutFeatureHighlightService';
import type { AdminAboutFeatureHighlight } from '../services/aboutFeatureHighlightService';
import { adminCategoryService } from '../services/categoryService';
import { AboutFeatureHighlightFormModal } from '../components/feature-highlights/AboutFeatureHighlightFormModal';
import { aboutFeatureHighlightKeys, categoryKeys } from '@/lib/queryKeys';
...
const SortableRow = ({
  ...
  item: AdminAboutFeatureHighlight;
  onEdit: (item: AdminAboutFeatureHighlight) => void;
  onDelete: (item: AdminAboutFeatureHighlight) => void;
...
export default function AboutFeatureHighlightsPage() {
  const queryClient = useQueryClient();
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalMode, setModalMode] = useState<'create' | 'edit'>('create');
  const [selectedItem, setSelectedItem] = useState<AdminAboutFeatureHighlight | null>(null);
  ...
  const { data: highlights = [], isLoading, isError } = useQuery({
    ...
    queryFn: adminAboutFeatureHighlightService.getAll,
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => adminAboutFeatureHighlightService.create(data, setUploadProgress),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aboutFeatureHighlightKeys.lists() });
      handleCloseModal();
    },
    onError: (error: any) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: string, data: any }) => adminAboutFeatureHighlightService.update(id, data, setUploadProgress),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aboutFeatureHighlightKeys.lists() });
      handleCloseModal();
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: adminAboutFeatureHighlightService.delete,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aboutFeatureHighlightKeys.lists() });
    }
  });
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, isActive }: { id: string; isActive: boolean }) => adminAboutFeatureHighlightService.updateStatus(id, isActive),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: aboutFeatureHighlightKeys.lists() });
    }
  });
  ...
  const reorderMutation = useMutation({
    mutationFn: adminAboutFeatureHighlightService.reorder,
  });
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      ...
      queryClient.setQueryData(aboutFeatureHighlightKeys.lists(), newItems);

      reorderMutation.mutate(
        newItems.map((item, index) => ({ _id: item._id, displayOrder: index + 1 })),
        {
          onError: () => queryClient.invalidateQueries({ queryKey: aboutFeatureHighlightKeys.lists() })
        }
      );
    }
  };
  ...
  const handleOpenEditModal = (item: AdminAboutFeatureHighlight) => {
    setModalMode('edit');
  ...
  const handleDelete = (item: AdminAboutFeatureHighlight) => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">About Page Highlights</h1>
          <p className="text-sm text-gray-500 mt-1">Manage the infinite product image carousel on the About page.</p>
        </div>
        <button
      ...
      <AboutFeatureHighlightFormModal
        isOpen={isModalOpen}
        mode={modalMode}
