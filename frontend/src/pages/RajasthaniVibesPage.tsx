...
const RajasthaniVibesTemplate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch rajasthani vibes products:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const HeroHeader = (
    <section className="pt-28 md:pt-32 pb-4 px-4 md:px-8" style={{ backgroundColor: 'var(--bg-page)' }}>
      <div className="max-w-7xl mx-auto flex flex-col justify-end">
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 mb-8">
          <Link to="/" className="font-sans text-[13px] font-medium transition-colors duration-200 hover:opacity-80" style={{ color: 'var(--text-secondary)' }}>Home</Link>
          <span style={{ color: 'var(--text-secondary)' }} className="font-sans text-[13px] font-medium">/</span>
          <span className="font-sans text-[13px] font-medium" style={{ color: 'var(--text-primary)' }}>Shop</span>
        </nav>
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
          <div className="flex flex-col gap-3">
            <h1 className="font-fraunces font-medium text-4xl md:text-5xl lg:text-6xl tracking-tight" style={{ color: 'var(--text-primary)' }}>
              Heritage in every thread, <em className="italic font-light">colour in every stitch.</em>
            </h1>
            <p className="font-sans text-[14px]" style={{ color: 'var(--text-secondary)' }}>
              Bags and women's accessories steeped in the rich textile traditions of Rajasthan — bold colour, intricate craft, timeless spirit.
            </p>
          </div>
          <div className="mb-1">
...
const RajasthaniVibesPage: React.FC = () => {
  return (
    <ThemeProvider category="rajasthani">
      <RajasthaniVibesTemplate />
    </ThemeProvider>
  );
};

export default RajasthaniVibesPage;
