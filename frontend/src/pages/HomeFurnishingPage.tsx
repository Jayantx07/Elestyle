...
const FILTER_TABS = [
  ...
  { id: 'throws', label: 'THROWS & BLANKETS' },
  { id: 'rugs', label: 'RUGS' },
...
const HomeFurnishingTemplate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch home furnishing products:', error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
          <div className="flex flex-col gap-3">
            <h1 className="font-fraunces font-medium text-4xl md:text-5xl lg:text-6xl tracking-tight" style={{ color: 'var(--text-primary)' }}>
              Spaces that breathe, <em className="italic font-light">crafted for comfort.</em>
            </h1>
            <p className="font-sans text-[14px]" style={{ color: 'var(--text-secondary)' }}>
              Elevate your living spaces with our curated collection of soft furnishings, from hand-loomed throws to artisan cushions.
            </p>
          </div>
...
const HomeFurnishingPage: React.FC = () => {
  return (
    <ThemeProvider category="furnishing">
      <HomeFurnishingTemplate />
    </ThemeProvider>
  );
};
