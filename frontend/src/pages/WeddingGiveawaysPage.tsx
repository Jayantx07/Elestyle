...
const WeddingGiveawaysTemplate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch wedding giveaways products:', error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
          <div className="flex flex-col gap-3">
            <h1 className="font-fraunces font-medium text-4xl md:text-5xl lg:text-6xl tracking-tight" style={{ color: 'var(--text-primary)' }}>
              Love, wrapped <em className="italic font-light">with intention.</em>
            </h1>
            <p className="font-sans text-[14px]" style={{ color: 'var(--text-secondary)' }}>
              Thoughtful, beautifully packaged gifts that carry the warmth of your celebration into every guest's home.
            </p>
          </div>
          <div className="mb-1">
...
const WeddingGiveawaysPage: React.FC = () => {
  return (
    <ThemeProvider category="wedding">
      <WeddingGiveawaysTemplate />
    </ThemeProvider>
  );
};

export default WeddingGiveawaysPage;
