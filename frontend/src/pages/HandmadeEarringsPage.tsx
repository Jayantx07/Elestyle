...
const HandmadeEarringsTemplate: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.error('Failed to fetch handmade earrings:', error);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
          <div className="flex flex-col gap-3">
            <h1 className="font-fraunces font-medium text-4xl md:text-5xl lg:text-6xl tracking-tight" style={{ color: 'var(--text-primary)' }}>
              Small details, <em className="italic font-light">loud statements.</em>
            </h1>
            <p className="font-sans text-[14px]" style={{ color: 'var(--text-secondary)' }}>
              Delicate, hand-formed earrings crafted from brass, terracotta, and natural stone. One-of-a-kind pieces for every mood.
            </p>
          </div>
          <div className="mb-1">
...
const HandmadeEarringsPage: React.FC = () => {
  return (
    <ThemeProvider category="earrings">
      <HandmadeEarringsTemplate />
    </ThemeProvider>
  );
};

export default HandmadeEarringsPage;
