...
const PUBLIC_CACHE_KEY = 'about-feature-highlights:public';
...
exports.getPublicAboutFeatureHighlights = async (req, res) => {
  try {
    const highlights = await cacheManager.wrap(PUBLIC_CACHE_KEY, 300, async () => {
      // Limit to max 50 for storefront performance
      const docs = await AboutFeatureHighlight.find({ isActive: true })
