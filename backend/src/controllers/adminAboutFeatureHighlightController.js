...
const AboutFeatureHighlight = require('../models/AboutFeatureHighlight');
...
const ADMIN_CACHE_KEY = 'about-feature-highlights:admin';
...
const uploadImageAsset = async (file) => {
  ...
  const folderPath = `ElleStyle/AboutFeatureHighlights`;
...
exports.getAdminAboutFeatureHighlights = async (req, res) => {
  try {
    const highlights = await cacheManager.wrap(ADMIN_CACHE_KEY, 120, async () => {
      const docs = await AboutFeatureHighlight.find()
...
exports.createAboutFeatureHighlight = async (req, res) => {
  ...
  try {
    ...
    const highlight = await AboutFeatureHighlight.create({
      altText,
    ...
    await cacheManager.clearPattern('about-feature-highlights');
    eventService.dispatchInvalidation('catalog', 'aboutFeatureHighlight', highlight._id);

    const populated = await AboutFeatureHighlight.findById(highlight._id).populate('category', 'name slug');
...
exports.updateAboutFeatureHighlight = async (req, res) => {
  let uploadedAsset = null;

  try {
    const highlight = await AboutFeatureHighlight.findById(req.params.id).populate('category', 'name slug');
    if (!highlight) {
...
exports.updateAboutFeatureHighlightStatus = async (req, res) => {
  try {
    const highlight = await AboutFeatureHighlight.findById(req.params.id);
    if (!highlight) {
      return res.status(404).json({ success: false, message: 'Feature highlight not found' });
    }
...
exports.reorderAboutFeatureHighlights = async (req, res) => {
  try {
    ...
    const updates = items.map((item, index) => {
      ...
      return AboutFeatureHighlight.findByIdAndUpdate(item._id, {
    ...
    await cacheManager.clearPattern('about-feature-highlights');
    eventService.dispatchInvalidation('catalog', 'aboutFeatureHighlight');
...
exports.deleteAboutFeatureHighlight = async (req, res) => {
  try {
    const highlight = await AboutFeatureHighlight.findById(req.params.id);
    if (!highlight) {
      return res.status(404).json({ success: false, message: 'Feature highlight not found' });
    ...
    await AboutFeatureHighlight.findByIdAndDelete(req.params.id);
    ...
    await cacheManager.clearPattern('about-feature-highlights');
    eventService.dispatchInvalidation('catalog', 'aboutFeatureHighlight', req.params.id);
